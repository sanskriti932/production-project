...
<body class="bg-gray-100 font-family-karla flex flex-col min-h-screen">
    <header class="header sticky top-0 bg-white shadow-md flex items-center justify-between px-8 py-2 z-50">
        ...
        <nav class="hidden lg:flex lg:w-6/12 justify-center">
            @include('layouts.cafefrontendcomponent.cafefrontnavbar')
        </nav>
    ...
    <form action="/search" class="flex items-center space-x-4 my-4">
        <div class="flex-grow">
        ...
        <button>
            <a href="#" class="hover:text-[#b3ab78] duration-200">
                <svg class="h-8 p-1" aria-hidden="true" focusable="false"
    ...
    <a href="{{url('cart')}}" class="nav-link">
        <svg class="h-8 p-1 hover:text-[#b3ab78] duration-200" aria-hidden="true" focusable="false"
    ...
    <div class="relative inline-block text-left">
        <button type="button" id="dropdownButton"
            class="inline-flex items-center justify-center w-8 h-8 p-1 text-black hover:text-[#b3ab78] focus:outline-none focus:text-[#b3ab78] z-10">
            <svg class="h-6 w-6 hover:text-[#b3ab78] duration-200" xmlns="http://www.w3.org/2000/svg" fill="none"
                viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M20 9V7a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                <circle cx="12" cy="9" r="3"></circle>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M16 21v-1a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v1"></path>
            </svg>
        </button>
        ...
        <div id="dropdownContent"
            ...
            <div class="py-1">
                ...
                @if (Route::has('login'))
                <a href="{{ route('login') }}"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#b3ab78]">Login</a>
                @endif

                @if (Route::has('register'))
                <a href="{{ route('register') }}"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#b3ab78]">Register</a>
                @endif
                @else
                <button @click="openDropdown = !openDropdown"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#b3ab78] focus:outline-none">
                    {{ Auth::user()->name }}
                </button>

                <a href="{{ route('logout') }}"
                    onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#b3ab78]">Logout</a>
                <a href="#"
                    onclick="event.preventDefault(); document.getElementById('profile-form').submit();"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#b3ab78]">My Profile</a>
                <a href="{{url('cust-order')}}"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#b3ab78]">My Orders</a>
                <form id="logout-form" action="{{ route('logout') }}" method="POST" class="hidden">
                    @csrf
                </form>
                @endguest
